...
class ZoneListModel {
    ...
    class Result {
        ...
        override fun toString(): String {
            return tsmAddress.toString() // What to display in the Spinner list.
        }

        override  fun equals(other: Any?): Boolean {
            if (other is ZoneListModel.Result ) {
                val c: ZoneListModel.Result  = other as ZoneListModel.Result
                if (c.tsmAddress.equals(tsmAddress) && c.id === id && c.tsmCode === tsmCode )
                    return true
            }
            return false
        }
